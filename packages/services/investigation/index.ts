...
class InvestigationService {
  ...
  async createSignozDashboard(investigationId: string, userId: string) {
    const [row] = await db
      .select()
      .from(investigationsTable)
      .where(eq(investigationsTable.id, investigationId))
    ...
    const { buildDashboardUrl, buildServiceOverviewDashboardPayload, createDashboard } = await import(
      "../signoz/dashboards-api"
    );

    const signozConfig = await resolveSignozConfig(row.organizationId);
    if (!signozConfig) {
      throw new Error(
        "SigNoz is not configured — connect in workspace settings or set SIGNOZ_CLOUD_URL / SIGNOZ_API_KEY",
      );
    }

    const serviceName = row.affectedServices[0] ?? getDefaultServiceName();
    const shortId = shortInvestigationId(row.id);
    const payload = buildServiceOverviewDashboardPayload(serviceName, {
      title: `${serviceName} — Service Overview (Case ${shortId})`,
      description: `Auto-generated by Evolvex for investigation "${row.title}" (Case ${shortId}): request rate, error rate, and p99 latency for ${serviceName}.`,
      tags: ["evolvex", "auto-generated", serviceName, shortId],
    });

    const created = await createDashboard(payload, signozConfig);
    const dashboardUrl = buildDashboardUrl(signozConfig, created.id);
    ...
    await insertTimelineEntry({
      ...
      occurredAt: new Date(),
      kind: "CHANGE",
      title: `SigNoz dashboard created · ${payload.title as string}`,
      detail: `Request rate, error rate, and p99 latency widgets for ${serviceName}.`,
      source: "signoz-dashboards-api",
      sourceRef: { dashboardId: created.id, dashboardUrl },
      sortOrder: maxSort + 1,
      metadata: { dashboardId: created.id, dashboardUrl, serviceName },
    });

    await recordAuditEvent({
      actorUserId: userId,
      action: "investigation.signoz_dashboard.created",
      resourceType: "investigation",
      resourceId: investigationId,
      metadata: { shortId, dashboardId: created.id, dashboardUrl, serviceName },
    });
